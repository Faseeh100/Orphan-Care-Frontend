...
//           <p className="text-gray-600 mt-2">Sign in to access the Orphan Care Admin Panel</p>
...
export default function LoginPage() {
  const router = useRouter();
  const [searchParams, setSearchParams] = useState<URLSearchParams | null>(null); // ✅ State for params
  const [justRegistered, setJustRegistered] = useState(false); // ✅ State for the flag
  const [formData, setFormData] = useState({
    email: '',
    password: ''
  });
  ...
  // ✅ Get search params ONLY in the browser (after component mounts)
  useEffect(() => {
